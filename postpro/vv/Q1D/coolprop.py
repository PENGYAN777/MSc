...
pt = 12e3 # total pressure
print("total pressure[Pa]:", pt)
tt = 300 # total compressibility factor
...
pd.DataFrame(P).to_csv('nozzle.csv', index_label = "Index", header  = ['pressure']) 
data = pd.read_csv("nozzle.csv", ",")
# append new columns
d =pd.DataFrame({'density': D, 'temperature': T, 'soundspeed': C,'Mach': M, 'Z': Z,'g': G,'Ar':Ar, 'x':x,'y':y})
